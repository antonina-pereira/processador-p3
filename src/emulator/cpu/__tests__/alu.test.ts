...
describe("ALU", () => {
  ...
  it("SUB: simple subtraction", () => {
    const { result, flags } = executeAlu("SUB", 10, 3);
    expect(result).toBe(7);
    expect(flags.Z).toBe(false);
  });

  it("SUB: zero result sets Z flag", () => {
    const { flags } = executeAlu("SUB", 5, 5);
    expect(flags.Z).toBe(true);
  });

  it("SUB: borrow clears carry flag", () => {
    const { flags } = executeAlu("SUB", 3, 5);
    expect(flags.C).toBe(false);
  });

  it("SUB: signed overflow", () => {
    const { flags } = executeAlu("SUB", 0x8000, 1); // -32768 - 1 = +32767
    expect(flags.O).toBe(true);
  });
  ...
  // -------------------------
  it("AND: bitwise and", () => {
    const { result } = executeAlu("AND", 0b1100, 0b1010);
    expect(result).toBe(0b1000);
  });

  it("OR: bitwise or", () => {
    const { result } = executeAlu("OR", 0b1100, 0b1010);
    expect(result).toBe(0b1110);
  });

  it("XOR: bitwise xor", () => {
    const { result } = executeAlu("XOR", 0b1100, 0b1010);
  ...
  it("CMP: sets flags based on a - b", () => {
    const { flags } = executeAlu("CMP", 10, 3);
    expect(flags.Z).toBe(false);
    expect(flags.N).toBe(false);
  });

  it("CMP: zero result", () => {
    const { flags } = executeAlu("CMP", 5, 5);
    expect(flags.Z).toBe(true);
  });

  it("CMP: negative result", () => {
    const { flags } = executeAlu("CMP", 3, 5);
